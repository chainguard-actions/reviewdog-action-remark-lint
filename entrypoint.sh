...
# Split INPUT_REMARK_ARGS and INPUT_REVIEWDOG_FLAGS into arrays to avoid
# shell metacharacter injection while still supporting multiple flags.
read -ra REMARK_ARGS_ARRAY <<< "${INPUT_REMARK_ARGS}"
read -ra REVIEWDOG_FLAGS_ARRAY <<< "${INPUT_REVIEWDOG_FLAGS}"

remark . "${REMARK_ARGS_ARRAY[@]}" 2>&1 |
  ...
  /tmp/reviewdog -f=remark-lint \
    ...
    -level="${INPUT_LEVEL}" \
    -tee \
    "${REVIEWDOG_FLAGS_ARRAY[@]}" || exit_val="$?"
